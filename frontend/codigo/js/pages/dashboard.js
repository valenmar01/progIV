            ...
            return `<tr>
                ...
                <td style="min-width:220px">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <span class="small text-muted" style="white-space:nowrap">${act}/${max}</span>
                    </div>
                </td>
                <td class="text-end">
                    <span class="badge rounded-pill bg-${colorEstado(c.estado)} px-3 py-2">${c.estado ?? '—'}</span>
                </td>
            </tr>`;
        }).join('')
...
const renderCursosActivos = (cursos) => {
    ...
    contenedor.innerHTML = `
        <div class="d-flex gap-2 flex-wrap mb-3" id="chips-filtro">
            ...
            ${estados.map(e => `
                <button class="btn btn-sm rounded-pill px-3 chip-filtro btn-outline-${colorEstado(e)}" data-estado="${e}" style="font-size:.8rem">${e}</button>
            `).join('')}
        </div>
...
const cargarDashboard = async () => {
    try {
        ...
        if (resCursos.ok) {
            ...
            if (el) el.textContent = total ?? '—';
            renderCursosActivos((cursos ?? []).filter(c => c.estado?.toLowerCase() !== ESTADO_EXCLUIDO));
        }
    } catch (e) {
